...
## media runtime split (vercel + render/railway)

this repo now uses a split runtime model:

- **next.js app** (frontend + lightweight api routes) on vercel
- **express media backend** in `/backend` for ffmpeg-heavy routes

rewritten paths in the next app:
...
- vercel / next app:
  ...
  - `NEXT_PUBLIC_MEDIA_BACKEND_URL=https://<your-media-backend-domain>` (enables direct browser fallback if proxy resets)
  - `OPENAI_API_KEY=...` (unchanged)
- media backend:
  - `PORT` (optional; defaults to 8787 locally)
  - `FFMPEG_PATH` (optional override)
  - `FFPROBE_PATH` (optional override)
  - `CORS_ORIGIN` (optional comma-separated allowlist; if unset, allows all)

### local run

1. `npm install`
2. `npm --prefix backend install`
3. set `MEDIA_BACKEND_URL=http://localhost:8787` and `NEXT_PUBLIC_MEDIA_BACKEND_URL=http://localhost:8787` in `.env.local`
4. run next app: `npm run dev`
5. run media backend: `npm run dev:backend`
